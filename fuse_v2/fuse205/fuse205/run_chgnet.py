...
def run_chgnet(atoms,n_opts=2,rel='',relaxer_opts={
	'fmax':[0.1,0.05],
	'steps':[250,750],
	'verbose':[True,True]
	}
	...
	if mode == 'relax':
		try:
			for i in range(n_opts):
				...
				prediction=relaxer.relax(temp_atoms,fmax=relaxer_opts['fmax'][i],steps=relaxer_opts['steps'][i],verbose=relaxer_opts['verbose'][i],relax_cell=cell)
