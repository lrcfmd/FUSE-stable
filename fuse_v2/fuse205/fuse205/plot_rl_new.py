...
def plot_rl_graph(actions,ini_pop,params_db,alpha,reinforce_table,theta_table,reinforce_id,reg_params,reinforce_verbosity,excluded_actions,h_type,
        ...
        try:
            ...
            for i in theta_list:
                ...
                for k in range(len(h_dict['1'])):
                    ...
                    prob=prob*100
                    prob=prob*actions.count(cur_action)
                    probs.append(prob)
    ...
    #print(all_probabilities['1'])
    try:
        for i in all_probabilities.keys():
            
            for k in all_probabilities[i].keys():
                #if not actions.count(i) == 0:
                mx_p=max(list(all_probabilities[i][k]))
                mi_p=min(list(all_probabilities[i][k]))
                #else:
                #	mx_p=max(list(all_probabilities[i][k]))
                #	mi_b=min(list(all_probabilities[i][k]))
