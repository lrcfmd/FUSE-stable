...
import random
import shutil
from fuse_stable.plot_results import plot_graph
...
def run_fuse(composition='',search=1,initial_gen='',max_atoms='',vac_ratio=4,ap_scale=1.0,
	 ...
	 search_gen=1,n_moves={1:18,2:18,3:3,4:3,5:3,6:2,7:1,8:1},r_moves={3:1,4:1,5:1,6:2,7:3,8:3},rmax='',T=0.02,
	 imax_atoms='',produce_steps=False,melt_threshold=500,
	 write_graph=True,swap_threshold=500,
	 new_structure_attempts=5000,calcs='',swap_back='',
	...
	if restart == True:
		try:
			...
			if system_type == 'ionic':
				import fuse_stable.bond_table_ionic
				bondtable=numpy.load("bondtable.npz",allow_pickle=True)
				bondtable=bondtable['bond_table'].item()
	...
	if initial_complete == 0:
		if serial == True: # run the structures one at a time
			...
			for i in range(0+(ncalc), initial_gen):
				...
				if ctype == 'vasp':
					try:
						atoms,energy,converged=run_vasp(atoms=atoms,vasp_opts=vasp_opts,kcut=kcut,produce_steps=produce_steps)
					except:
						#print('except')
						converged = False
						energy = 1.e20	
