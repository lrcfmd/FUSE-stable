...
import random
import shutil
from fuse104.plot_results import plot_graph
from fuse104.check_prev_structure import *
from ase.build import sort
from fuse104.run_multiple_calculators import run_calculators
import pandas
...
def run_fuse(composition='',search='',initial_gen='',max_atoms='',vac_ratio=4,ap_scale=1.0,
	 ...
	 write_graph=False,new_structure_attempts=5000,calcs='',qe_opts='',
	 gulp_command='gulp < gulp.gin > gulp.got',record_data_log=True,
	 write_graph_end=True,gulp_timeout=''):
	...
	if restart == True:
		try:
			...
			if system_type == 'ionic':
				import fuse104.bond_table_ionic
				bondtable=numpy.load("bondtable.npz",allow_pickle=True)
				bondtable=bondtable['bond_table'].item()				
			
			if system_type == 'neutral':
				import fuse104.bond_table_atomic
				bondtable=numpy.load("bondtable.npz",allow_pickle=True)
				bondtable=bondtable['bond_table'].item()
	if restart == False:
		if system_type == 'ionic':
	...
	o.close()
	o=open("output.txt",'a')
	### now we can move onto using the searching routines ###############
	start=1
	if initial_complete == 1:
		if search != 3:
			while r < rmax:
				...
				if generation_complete==True:
						...
						while built < search_gen:
							#print("built",built)
							if search == 1:
								new_structure=make_basin_move(current_structure,atoms_per_fu,fu,vac_ratio,max_fus,system_type,composition,ap,cubic_solutions,tetragonal_solutions,hexagonal_solutions,orthorhombic_solutions,monoclinic_solutions,moves=moves,max_atoms=max_atoms,initial_population=initial_population,search_structures=search_structures)
